...
def render_process_panel():
    ...
    st.caption(
        ...
        "🧠 ตัวทำนายบนบอร์ดคือ **thesis_ML_v2** (เทรนด้วยข้อมูล พ.ศ. 2565–2566 "
        "ตามขอบเขต ethics approval — ตรึงไว้ให้ตรงกับเล่มวิจัย · "
        "การนำเข้านี้เติมเฉพาะฐานสถิติ ไม่กระทบโมเดล)"
    )
    # (กล่องแจ้ง ethics lock ถอดออกจาก UI 19 ก.ค. 2026 ตามคำสั่งมุคกี้ —
    #  ตัวล็อกจริงยังทำงานอยู่: ส่วน fine-tune ไม่มีในโค้ด panel นี้แล้ว
    #  รายละเอียดดู docs/ETHICS_LOCK_2026-06-10.md)
    ...
    if st.button("③ ▶️ ประมวลผลทั้งหมด (อัปเดต dashboard + สถิติ)", type="primary",
        ...
        #    ข้อมูลปี 2568-2569 อยู่นอกขอบเขต ethics approval (เทรนได้เฉพาะ 2565-2567)
