...
# ส่วนที่คนแก้จริง ๆ (เวลาทำนาย/พยาบาล/หมายเหตุ) ยังรับมาตามปกติ ไม่ทิ้ง
...
def _or_board_demo():
    ...
    → คง pattern แพทย์ประจำวัน · mask ครบ (ชื่อแพทย์จำลอง เช่น "โกมินทร์ กุมาร" /
...
def _demo_surgeons_by_specialty():
    ...
    อยู่แล้ว (เช่น "วีระ ประสงค์สุข" = SURG_003) เคสที่ผู้ทรงพิมพ์เพิ่มเองจึงยังทำนายด้วย
...
def _board_fragment():
    ...
    def _board_help_md():
        st.markdown(
            ...
            "- **⏭️ คิวถัดไป** (แถบใต้การ์ดเคสคิวถัดไปของแต่ละห้อง): AI คำนวณ"
            "เวลาที่ควร **ตามผู้ป่วยขึ้นมารอ** ให้รอหน้าห้องไม่เกิน 1 ชั่วโมง "
            "ตัวเลขขยับตามสถานการณ์จริงเอง — ถึงเวลาก็ไปตามผู้ป่วยแล้วกดรับเข้า\n"
            "- ถ้าในห้องผ่าตัดกดกระดิ่ง **🔔** (ใกล้ผ่าเสร็จแล้ว) แถบจะเปลี่ยนเป็น "
            "สีเขียว **\"ในห้องผ่าตัดแจ้ง : เรียกเคสนี้มารอได้เลย\"** พร้อมเสียงติ๊ง "
            "= ตามผู้ป่วยได้ทันที ไม่ต้องรอถึงเวลา\n"
            ...
            "จำนวนกี่เคสในข้อมูลที่ใช้พัฒนา (พ.ศ. 2565–2566) ยิ่งมากยิ่งหนักแน่น · "
...
def _room_focus_fragment(room_no):
    ...
    if cur is not None:
        ...
        st.markdown(
            ...
            f'<span>ทำนาย ~{eff} นาที</span></div>'
            f'</div>', unsafe_allow_html=True)
        # ✏️ แก้เวลาทำนายใช้ห้อง — ย้ายมาอยู่เหนือปุ่มผ่าเสร็จ (9 ส.ค. 2026
        ...
        with st.container(border=True):
            ...
            with _ecol1:
                st.markdown('<span style="font-size:18px;font-weight:600;'
                             'color:#0f172a;">✏️ แก้เวลาทำนายใช้ห้อง</span>',
                             unsafe_allow_html=True)
