    ...
    with st.expander("📤 อัปโหลดตารางผ่าตัดวันนี้ (CSV)", expanded=False):
        ...
        if st.checkbox("🗑️ ล้างกระดานวันนี้ (สำหรับลบเคสทดสอบ) 🔒",
                       ...
                       help="ลบเคสทั้งหมดของวันนี้ออกจากบอร์ด+บอร์ดกลาง (ทุกเครื่อง) "
                            "— เฉพาะผู้ดูแล · ไม่กระทบสถิติย้อนหลัง"):
            # 👤 production 19 ก.ค. 2026: เช็กบทบาทจากหน้า login — ไม่ถามรหัสซ้ำ
            if st.session_state.get('role') != 'admin':
                st.caption("🔒 การล้างกระดานทำได้เฉพาะผู้ดูแล — ออกจากระบบ "
                           "แล้วเข้าใหม่ด้วยรหัสผู้ดูแล")
            else:
...
def _board_fragment():
    ...
    with st.expander("❓ วิธีใช้บอร์ดตารางผ่าตัด", expanded=False):
        st.markdown(
            ...
            "---\n"
            "**ระดับความมั่นใจของการทำนาย**\n\n"
            "1. **ระดับความมั่นใจ (สูง/ปานกลาง/ต่ำ)** — ประเมินจากความคุ้นเคย"
            "ของแบบจำลองต่อข้อมูลนำเข้า 3 องค์ประกอบ ได้แก่ หัตถการ · ศัลยแพทย์ · "
            "กลุ่มหัตถการ\n"
            "   - *สูง* = แบบจำลองรู้จักครบทั้งสามองค์ประกอบ\n"
            "   - *ปานกลาง* = รู้จัก 2 ใน 3 องค์ประกอบ\n"
            "   - *ต่ำ* = รู้จักน้อยกว่านั้น (การทำนายอิงค่ากลางเป็นหลัก)\n"
            "2. **จำนวนเคสอ้างอิง (จาก N เคส)** — จำนวนเคสของหัตถการนั้น"
            "ในข้อมูลที่ใช้พัฒนาแบบจำลอง (พ.ศ. 2564–2566)\n\n"
