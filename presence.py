...
def chips_html() -> str:
    ...
    🎨 11 ส.ค. 2026 (มุคกี้เคาะรอบสอง): บนหัวแอปเหลือแค่ "แถบสีเขียว" เปล่า ๆ
       ไม่ต้องมีตัวเลขให้รกหัวจอ · จำนวนไปอยู่ใน tooltip ตอนเอาเมาส์จ่อแทน
       (ใช้ attribute title ล้วน ๆ ไม่ง้อ JS — ทำงานได้ทุกเบราว์เซอร์)
    ...
    if _total <= 0:
        return ''
    # นับจริงเป็น "เครื่อง/จอ" แต่สื่อกับผู้ใช้ว่า "ท่าน" ตามที่เคาะไว้
    _tip = f"มีผู้ใช้งานในระบบ OR Flow ตอนนี้ {_total} ท่าน"
    return ('<span class="or-chip" '
            f'title="{_tip}" aria-label="{_tip}" role="img" '
            'style="background:#1b7f4b;border-color:#1b7f4b;'
            'padding:5px 18px;cursor:help;">&nbsp;</span>')
